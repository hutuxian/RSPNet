...
class BasicConv3D(nn.Layer):
    def __init__(self,
        ...
        self.conv3d = nn.Conv3D(in_channel, out_channel, kernel_size=kernel_size,
                                     stride=stride, padding=padding, bias_attr=use_bias)
        if use_bn:
            self.bn = nn.BatchNorm3D(out_channel, epsilon=1e-3, momentum=0.001)
...
class sep_inc(nn.Layer):
    ...
    def forward(self, x):
        ...
        out = paddle.concat([out_0, out_1, out_2, out_3], axis=1)
        return out


class S3D_G(nn.Layer):
